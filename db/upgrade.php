...
/**
 * Theme UFPel upgrade script - With compact logo removal.
 *
 * @package    theme_ufpel
...
function xmldb_theme_ufpel_upgrade($oldversion) {
    ...
    if ($oldversion < 2025090100) {
        ...
        try {
            ...
            // Cache definitions not found (will be created on first use)
        }
    ...
    // Version 2025120300 - Remove compact logo functionality
    if ($oldversion < 2025120300) {
        // Remove all compact logo related settings
        $compactlogoSettings = [
            'compactlogo',
            'logodisplaymode'
        ];
        
        foreach ($compactlogoSettings as $setting) {
            if (get_config('theme_ufpel', $setting) !== false) {
                unset_config($setting, 'theme_ufpel');
            }
        }
        
        // Clear all template caches to ensure updated templates are used
        $cachedir = $CFG->dataroot . '/localcache/mustache/-1/ufpel';
        if (is_dir($cachedir)) {
            // Remove all cached templates
            $iterator = new RecursiveIteratorIterator(
        ...
        upgrade_plugin_savepoint(true, 2025120300, 'theme', 'ufpel');
    }
